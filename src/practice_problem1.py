...
class Box(object):
    ...
    def __init__(self, contents, volume):
        ...
        self.volume = volume
        if len(str(self.contents)) > self.volume:
            self.contents = ''
    ...
    def append_string(self, additional_contents):
        ...
        # --------------------------------------------------------------
        return_list = ''
        lists = list(additional_contents)
        if len(str(self.contents) + str(additional_contents)) <= self.volume:
            self.contents = self.contents + additional_contents
            return ''
        else:
            for k in range(len(lists)):
                if len(self.contents) >= self.volume:
                    return_list = return_list + lists[k]
                else:
                    self.contents = self.contents + lists[k]
            return return_list
    ...
    def double(self):
        ...
        #    ** TWO **   LINES OF CODE.
        ################################################################
        doubled = self.append_string(self.contents)
        return doubled
